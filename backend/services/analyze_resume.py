...
SYSTEM_PROMPT = """You are **ResumeFit**, an advanced résumé-analysis agent hired to deliver a rigorous, 360° evaluation comparing a candidate's résumé to a target job description. Adopt the tone of an experienced career strategist: professional, concise, and highly actionable.
...
def generate_resume_analysis(raw_job_description: str, raw_resume: str, extracted_job_keywords: dict = None, extracted_resume_keywords: dict = None):
    """
    Generates a full resume analysis report in Markdown format using Gemini model.
    """
    extras = ""
    if extracted_job_keywords:
        extras += "\n\nExtracted Job Keywords (JSON):\n" + json_pretty(extracted_job_keywords)
    if extracted_resume_keywords:
        extras += "\n\nExtracted Resume Keywords (JSON):\n" + json_pretty(extracted_resume_keywords)
